...
帧范围编辑器是一个专为视频标注任务设计的强大工具，允许用户对一系列视频帧执行批量操作。当处理需要对多个连续帧应用相同操作的大型视频数据集时，此工具特别有用。

**功能特点：**
- **批量操作**：对指定范围的帧执行操作，而不是逐个处理帧
- **多种操作类型**：支持多种操作类型，包括删除所有标注、移除选定对象或添加选定对象
- **备份创建**：可选择在执行操作前创建标注文件的备份
- **帧范围验证**：输入验证确保起始帧不大于结束帧

**支持的操作：**
1. **删除所有标注**：删除指定帧范围内的所有标注文件（JSON）
2. **移除选定对象**：在帧范围内移除与当前选定形状匹配的特定对象
3. **添加选定对象**：将当前选定的形状添加到指定范围内的帧中

**使用方法：**
1. 在画布上选择一个或多个要操作的形状（用于移除或添加操作）
2. 点击菜单栏中的`工具` > `帧范围编辑器`
3. 在弹出的对话框中：
   - 输入起始帧号
   - 输入结束帧号
...
**注意事项：**
- 帧号基于视频帧命名约定（例如 frame_00001.jpg）
- 备份文件存储在同一目录的`_backup`文件夹中
- 对于移除/添加操作，打开对话框前必须在画布上选择形状
- 操作将处理指定范围内的所有帧，包括可能没有标注的帧

该工具通过消除手动逐帧处理的需要，显著加快了视频标注工作流程。
