...
class YOLOv5_CLS(Model):
    ...
    def det_pre_process(self, input_image, det_net):
        """Detection
        ...
        if len(image.shape) == 3:
            image = image[None]
        inputs = det_net.get_inputs()[0].name
        outputs = det_net.run(None, {inputs: image})[0]

        return image, outputs

    def det_post_process(self, img_src, img_processed, outputs):
        """Detection
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                output_info = {
                    ...
                    "x2": x2,
                    "y2": y2,
                    "label": self.det_classes[int(cls)],
                    "score": conf,
                }
    ...
    def predict_shapes(self, image, image_path=None):
        ...
        processed_img, detections = self.det_pre_process(image, self.det_net)
        boxes = self.det_post_process(image, processed_img, detections)
