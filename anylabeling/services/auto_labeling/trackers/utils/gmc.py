...
class GMC:
    ...
    def __init__(
        self, method: str = "sparseOptFlow", downscale: int = 2
    ) -> None:
        """
    ...
    def applyFeatures(
        self, raw_frame: np.array, detections: list = None
    ) -> np.array:
        """
        ...
        inliers = (
            spatialDistances - meanSpatialDistances
    ...
    def applySparseOptFlow(self, raw_frame: np.array) -> np.array:
        ...
        # Find rigid matrix
        if (prevPoints.shape[0] > 4) and (
            prevPoints.shape[0] == prevPoints.shape[0]
        ):
