...
class Transformer(nn.Module):
    def __init__(
        ...
        if not isinstance(num_patterns, int):
            Warning(
                "num_patterns should be int but {}".format(type(num_patterns))
            )
            self.num_patterns = 0
    ...
    def forward(
        ...
        if self.two_stage_type == "standard":  # 把encoder的输出作为proposal
            ...
            refpoint_embed_ = (
                self.refpoint_embed.weight[:, None, :]
                .repeat(1, bs, 1)
                .transpose(0, 1)
            )  # nq, bs, 4
...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            pos_scale = (
                self.query_scale(output) if self.query_scale is not None else 1
            )
